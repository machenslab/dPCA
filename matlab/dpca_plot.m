   ...
   && ~isempty(options.marginalizationNames)
    ...
    for m = intersect(1:4, unique(options.whichMarg(componentsToPlot)))        
        ...
        annotation('rectangle', [offsetX-0.005 pos(2) 0.015 pos(4)], ...
            'EdgeColor', 'none', 'FaceColor', options.marginalizationColours(margRowSeq(m),:));
        
        annotation('textarrow', offsetX*[1 1], yposs(m)*[1 1], ...
            'string', options.marginalizationNames{margRowSeq(m)}, ...
